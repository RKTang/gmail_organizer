...
Simple desktop app to keep folders neat by sorting files into category subfolders.

## Features

- Add one or more folders to manage
- Preview planned file moves before applying
- Apply moves safely with name conflict handling
- Undo the last apply run
- Optional auto mode (polling every 30 seconds)
...
```json
{
  ...
  "rules": [
    { "match": [".png", ".jpg"], "target": "Images" },
    { "match": [".pdf", ".docx"], "target": "Documents" }
  ],
  "default_target": "Misc",
...
- Only files directly under each configured folder are organized (not recursive).
- Undo restores files from the latest run if destination files still exist.
